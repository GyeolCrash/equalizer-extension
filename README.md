# Graph-based Audio Equalizer - Chrome Extension

인터랙티브 그래프 기반 동적 이퀄라이저 Chrome 확장 프로그램입니다. Pro-Q 스타일의 그래프에서 직접 클릭하여 필터를 추가하고, 마우스 드래그로 주파수 및 게인을 조정하여 웹 브라우저의 모든 오디오 스트림에 실시간으로 이퀄라이제이션을 적용합니다.

## 주요 기능

- 🎯 **그래프 기반 필터 추가/제거**: 그래프 클릭(좌) → 필터 추가 / 우클릭 → 제거
- 🎚️ **무제한 동적 노드**: 필요한 만큼 필터를 추가하고 제거 (Pro-Q 스타일)
- 📊 **실시간 FFT 주파수 시각화**: 실시간 오디오 스펙트럼을 그래프 배경에 표시
- 🎯 **세밀한 주파수 제어**: 
  - 게인(Gain): -12dB ~ +12dB
  - Q (대역폭): 0.1 ~ 10.0
  - 주파수: 20Hz ~ 20kHz (로그 스케일)
  - 필터 타입: Peaking, Low Shelf, High Shelf, Low Pass, High Pass
- 🔊 **마스터 게인 컨트롤**: 전체 볼륨 조정
- 🎨 **직관적인 UI**: FabFilter Pro-Q 스타일 그래프와 실시간 피드백
